...
inline constexpr const int kStateEncodingSize = 1220; // 50 planes * 1 * 24 points + 20 global scalars
...
class BackgammonState : public State {
 ...
 private:
  ...
  int GetPrimeLength(Player p_check, int b) const;
  float GetBlockadeDensity(Player perspective, Player p_check, int relative_idx, int r_dir) const;
